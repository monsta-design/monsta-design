...
@Directive({
  ...
  exportAs: "nsDraggable"
})
export class NSDraggableDirective
  implements OnInit, OnDestroy, OnChanges, AfterViewInit {
  private allowDrag = true;
  private moving = false;
  private orignal: NSPosition = null;
  private oldTrans = new NSPosition(0, 0, 0, 0, 0);
  private tempTrans = new NSPosition(0, 0, 0, 0, 0);
  private currTrans = new NSPosition(0, 0, 0, 0, 0);
  private oldZIndex = "";
  private _zIndex = "";
  ...
  @Output() movingOffset = new EventEmitter<NSIPosition>();

  /** Emit position offsets when put back */
  @Output() endOffset = new EventEmitter<NSIPosition>();
  ...
  resetPosition() {
    if (NSPosition.isIPosition(this.position)) {
  ...
  onMouseDown(event: MouseEvent | TouchEvent) {
    ...
    this.orignal = NSPosition.fromEvent(event, this.getDragEl());
    this.pickUp();
  }
  ...
  onMouseMove(event: MouseEvent | TouchEvent) {
    if (this.moving && this.allowDrag) {
      ...
      this.moveTo(NSPosition.fromEvent(event, this.getDragEl()));
    }
  }
}
